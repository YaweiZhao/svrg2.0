...
{  
  ...
  for (ite = 0; ite < ite_num; ++ite) {
    ...
    AzTimeLog::print2Logfile(log,strlen(log),"log.txt","a+");
    AzTimeLog::print2Logfile(log,strlen(log),"weights_log.txt","a+");
...
cout<<"-----------------------> ix#;"<<ix<<endl;      
      ...
      AzTimeLog::print2Logfile(log2,strlen(log2),"log.txt","a+");
      str1 = "\n>Inner loop:";
      str2 = ":\n";
      string str0 = str1 +itnum+str2;
      const char* log_weights = str0.c_str();
      AzTimeLog::print2Logfile(log_weights,strlen(log_weights),"weights_log.txt","a+");
      const int dim = m_trn_x->rowNum();
      double* weights = this->getWeights(dim); 
