...
int rj_compare_Ent(const void *v1, const void *v2); 
int rj_compare_EntTmp(const void *v1, const void *v2); 
...
/*------------------------------------------------------------------*/
AzStrPool::AzStrPool(int init_num, int avg_data_len)
{
  initialize(); 
...
#if 0 
    data = (AzByte *)file->readBytes(data_len); 
#endif 
  }
...
void AzStrPool::_swap()
{
  ...
  for (ex = 0; ex < ent_num; ++ex) {
    AzSpEnt *ep = &ent[ex]; 
    AzFile::swap_long(&ep->offs); 
    AzFile::swap_long(&ep->len); 
    AzFile::swap_long(&ep->count); 
    AzFile::swap_long(&ep->value); 
  }
}
...
/*------------------------------------------------------------------*/
int AzStrPool::write(AzFile *file) 
{
  int io_len = 0; 

  io_len += file->writeInt(ent_num); 
  io_len += file->writeInt(data_len); 
  io_len += file->writeBool(isCommitted); 

  if (data_len > 0) {
    io_len += file->writeBytes(data, data_len); 
  }
...
/*------------------------------------------------------------------*/
int AzStrPool::getCount(int ent_no) const
{
...
void AzStrPool::commit() 
{
  ...
  qsort(ent, ent_num, sizeof(ent[0]), rj_compare_Ent); 
  ...
  for (ex = 0; ex < ent_num; ) {
    ...
    int count = 0; 
    for ( ; ex < ent_num; ++ex) {
      if (rj_compare_Ent(&ent[ex], &ent[new_id]) != 0) {
        break;
      }
                         ...
                         int bytes_len) const
{
  ...
  for ( ; ; ) {
    ...
    int cmp = rj_compare_Ent(&inp, &ent[mx]); 
...
/*------------------------------------------------------------------*/
int AzStrPool::inc_data(int min_inc)
{
  int inc = MIN(1024*1024*10, MAX(init_data_len, data_len)); 
  inc = MAX(inc, min_inc); 
  return inc; 
}
...
void AzStrPool::reduce(int min_count)
{
  ...
  AzBytArr s_temp; 
  AzByte *temp_data = s_temp.reset(data_len, 0); 
...
int rj_compare_EntTmp(const void *v1, const void *v2) 
{
  AzENT_TMP *p1 = (AzENT_TMP *)v1;  
  AzENT_TMP *p2 = (AzENT_TMP *)v2;  

  return rj_compare_Ent(p1->ent_ptr, p2->ent_ptr); 
}
