...
/*                      AzTimeLog                             */
/***************************************************************/
/*AzTimeLog::print2Logfile
 *print the information to a log file which is used to do data analysis*/

void AzTimeLog::print2Logfile(const char text[], const int len, const char* fn, const char* mod)
{
  AzFile af(fn);
  af.open(mod);
  ...
  //string time_str(ctime(&ltime));
  //string msg(text);
  //string output = time_str+msg;
  char* str_time = ctime(&ltime);
  const int length = strlen(str_time)+strlen(text)+1;
  char output[length];	
  strcpy(output,str_time);
  strcat(output,text);
  
  af.writeBytes(output,length);
}

/*AzTimeLog::printDouble2Logfile
 *print the information to a log file which is used to do data analysis*/

void AzTimeLog::printDouble2Logfile(const double weights[],  const int len,const char* fn, const char* mod)
{
  const int size = len * sizeof(double);
  char weights_str[size];
  for(int i=0;i<len;i++)
    sprintf(&weights_str[i*sizeof(double)],"%f",weights[i]);
  AzFile af(fn);
  af.open(mod);
  af.writeBytes(weights_str,size);
}
